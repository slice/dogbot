...
class Utility(Cog):
    ...
    async def steal_emoji(self,
        ...
        if not emoji.name and not name:
            return await ctx.send(
                'You must provide the name for the stolen emoji.')
        ...
        try:
            async with ctx.bot.session.get(emoji_url) as emoji_resp:
                ...
                emoji = await ctx.guild.create_custom_emoji(
                    name=name or emoji.name, image=emoji_bytes)
    ...
    async def define(self, ctx, *, word: str):
        ...
        search_params = {'q': word, 'limit': 3}

        # search word
        async with ctx.bot.session.get(
            api_base + '/search/en', params=search_params,
        ...
        # get word definition
        async with ctx.bot.session.get(
                api_base + f'/entries/en/{utils.urlescape(word_id)}',
                headers=headers) as resp:
            # grab results
            try:
                results = (await resp.json())['results']
                ...
                return await ctx.send(
                    'Hmm, I couldn\'t decode the results from Oxford.')
            lexical = results[0]['lexicalEntries'][0]

            if 'pronunciations' in lexical:
                pronun = ','.join(p['phoneticSpelling']
                                  for p in lexical['pronunciations']
        ...
        embed = discord.Embed(
            title=utils.truncate(lexical['text'], 256),
            description=utils.truncate(def_text, 2048))
        if examples:
            examples_text = '\n'.join(
                f'\N{BULLET} {example["text"]}' for example in examples)
            embed.add_field(
                name='Examples', value=utils.truncate(examples_text, 1024))
        embed.set_footer(text=pronun)
        await ctx.send(embed=embed)

    @define.error
    async def define_error(self, ctx, err):
        if isinstance(err, commands.DisabledCommand) or isinstance(
    ...
    async def weather(self, ctx, *, place: str):
        ...
        try:
            obv = await ctx.bot.loop.run_in_executor(
                None, self.owm.weather_at_place, place)
        ...
        temp = (
            f'{c["temp"]}°C ({c["temp_min"]}°C \N{EM DASH} {c["temp_max"]}°C)\n'
    ...
    @commands.command()
    @commands.guild_only()
    async def poll(self, ctx, conclude_on_votes: int, title: str,
                   *choices: str):
        """
        Creates a poll.
        ...
        if len(choices) > 9:
            raise commands.errors.BadArgument(
                'Too many choices! There is a maximum of nine.')

        if conclude_on_votes < 1:
            raise commands.errors.BadArgument(
                'Invalid `conclude_on_votes` parameter. It must be '
                'greater than zero.')
        ...
        if not choices:
            ...
            raise commands.errors.BadArgument(
                'You need to specify some choices.')

        member_count = len(ctx.guild.members)
        if conclude_on_votes > member_count:
            raise commands.errors.BadArgument(
                'There aren\'t that many people on the server. '
                f'({conclude_on_votes} > {member_count})')

        embed = discord.Embed(
            title=f'Poll by {ctx.author}: {title}',
        ...
        while True:
            ...
            reaction, adder = await self.bot.wait_for(
                'reaction_add', check=message_check)
            ...
            if len(reaction.emoji) != 2 or reaction.emoji[1] != '\u20e3':
                logger.debug(
                    'Ignoring invalid poll reaction -- reaction=%s, len(reaction)=%d',
        ...
        winning_choice = choices[max(
            poll_results.items(), key=operator.itemgetter(1))[0]]
        poll_summary = '\n'.join(
            f'{choices[index]} \N{EM DASH} **{votes}** vote(s)'
        ...
        for sense in result['senses']:
            ...
            if 'english_definitions' in sense:
                ...
                if not sense['restrictions']:
                    ...
                    embed.add_field(
                        name=restr, value=sense_value, inline=False)
        await ctx.send(embed=embed)
    ...
    async def jumbo(self, ctx, emoji: converters.FormattedCustomEmoji):
        ...
        wrapper_embed = discord.Embed(
            title=f':{emoji.name}: \N{EM DASH} `{emoji.id}`')
    ...
    async def pick(self, ctx, *args):
        ...
        if not args:
            return await ctx.send(
                '\N{CONFUSED FACE} I can\'t choose from an empty list!')
