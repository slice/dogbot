...
async def export_image(ctx: DogbotContext, image: Image, filename: str):
    with BytesIO() as bio:
        # Export the image.
        coro = ctx.bot.loop.run_in_executor(None,
                                            functools.partial(
...
class Meme:
    def __init__(self, source, ctx, *, text_size=32, font_file=None):
        ...
        self.font = ImageFont.truetype(
            font_file or 'resources/font/SourceSansPro-Regular.ttf', text_size)
        ...
        async with ctx.typing():
            ...
            for step in recipe['steps']:
                if 'place' in step:
                    ...
                    meme.text(step['text'], step['x'], step['y'],
                              step.get('max_width', 10e9),
                              step.get('fill', (0, 0, 0)))
    ...
    async def cache(self, url, size=None):
        ...
        # download image
        image = self.image_cache[url] = await download_image(
            self.ctx.bot.session, url)
...
class Memes(Cog):
    async def __error(self, ctx, error):
        if isinstance(error, commands.CommandInvokeError):
            ...
            await ctx.send(
                'Something went wrong processing your image. Sorry about that!'
            )
    ...
    async def jpeg(self, ctx, image_source: converters.Image):
        ...
        with BytesIO() as output:
            await ctx.bot.loop.run_in_executor(None,
                                               functools.partial(
                                                   im.save,
                                                   output,
                                                   format='jpeg',
            ...
            async with ctx.typing():
                async with ctx.bot.session.get(url) as resp:
                    with BytesIO(await resp.read()) as bio:
                        await ctx.send(
                            file=discord.File(filename='orly.png', fp=bio))
    ...
    @commands.cooldown(1, 3, commands.BucketType.user)
    async def whowouldwin(self, ctx, left: converters.Image, left_text,
                          right: converters.Image, right_text):
        """Who would win?"""
        await Meme.recipe(
            ctx, {
                'image':
                'resources/memes/whowouldwin.png',
                'render_as':
                'whowouldwin.png',
                'cache': [(left, (312, 277)), (right, (350, 250))],
                'steps': [{
                    'place': (left, (24, 200))
                }, {
                    'place': (right, (434, 210))
    ...
    @commands.command()
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def youcantjust(self, ctx, coolio: converters.Image, *,
                          text: commands.clean_content):
        """You can't just..."""
        await Meme.recipe(
            ctx, {
                'image':
                'resources/memes/you_cant_just.png',
                'render_as':
                'you_cant_just.png',
                'additional': {
                    'text_size': 20
    ...
    @commands.command(
        aliases=['handicap'], )
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def handicapped(self, ctx, image_source: converters.Image, *,
                          text: commands.clean_content):
        """Sir, this spot is for the handicapped only!..."""
        await Meme.recipe(
            ctx, {
                'image':
                'resources/memes/handicap.png',
                'render_as':
                'handicapped.png',
    ...
    @commands.command()
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def floor(self, ctx, image_source: converters.Image, *,
                    text: commands.clean_content):
        """
        The floor is...
    ...
    async def wacky(self, ctx, image_source: converters.Image = None):
        ...
        image_source = image_source or ctx.message.author.avatar_url_as(
            format='png')
        ...
        avatar_im = await self.bot.loop.run_in_executor(
            None, enhancer.enhance, 50)
