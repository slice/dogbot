...
class Info(Cog):
    ...
    async def permissions(self, ctx):
        ...
        max_perm_length = max(
            len(checks.beautify_permission_name(p[0])) for p in perms)

        for attr, value in perms:
            indicator = '\U00002705' if value else '\U0000274c'
            paginator.add_line(
                f'{checks.beautify_permission_name(attr): <{max_perm_length}} {indicator}'
            )
    ...
    async def profile_describe(self, ctx, color: discord.Color, *,
                               description):
        """Sets your profile description and color. Supporter only."""
        if len(description) > 1024:
            return await ctx.send(
                'That description is too long. There is a maximum of 1024 characters.'
            )
        async with ctx.acquire() as conn:
    ...
    async def server(self, ctx):
        ...
        num_online = len(
            list(
                filter(lambda m: m.status is discord.Status.online,
                       g.members)))
        num_offline = len(
            list(
                filter(lambda m: m.status is discord.Status.offline,
                       g.members)))
        embed.add_field(
            name='Members',
            value=SERVER_INFO_MEMBERS.format(
                cm(total_members), cm(num_online), cm(num_offline),
                round((num_online / total_members) * 100, 2), humans, bots,
                round(humans / bots, 2)))

        embed.add_field(
            name='Count',
            value=SERVER_INFO_COUNT.format(
                cm(len(g.roles)), cm(len(g.text_channels)),
                cm(len(g.voice_channels)), cm(len(g.channels))))
        ...
        # guild owner
        embed.set_footer(
            text=f'Owned by {g.owner}', icon_url=g.owner.avatar_url)
    ...
    async def invite_info(self, ctx, invite: discord.Invite):
        ...
        embed = discord.Embed(
            title=f'Invite for {invite.guild.name} (`{invite.guild.id}`)')
        embed.add_field(
            name='Channel',
            value=f'{invite.channel.name} (`{invite.channel.id}`)',
            inline=False)
        if invite.inviter:
            embed.add_field(
                name='Inviter',
                value=describe(invite.inviter, mention=True),
                inline=False)
        await ctx.send(embed=embed)
