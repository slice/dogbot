...
async def is_publicly_visible(bot: Dogbot,
...
class Modlog(Cog):
    ...
    def modlog_msg(self, msg: str) -> str:
        ...
        """
        return '`[{0.hour:02d}:{0.minute:02d}]` {1}'.format(
    ...
    async def log(self,
                  ...
                  text: str,
                  *,
                  do_not_format: bool = False) -> discord.Message:
        """
        ...
        """
        return await self.bot.send_modlog(guild, text if do_not_format else
                                          self.modlog_msg(text))
    ...
    async def on_guild_emojis_update(self, guild: discord.Guild,
        ...
        await self.log(guild,
                       f'\N{FRAME WITH PICTURE} Emoji updated: {differences}')
    ...
    async def on_voice_state_update(self, member: discord.Member,
        ...
        async def send(m):
            await self.log((before.channel.guild
                            if before.channel else after.channel.guild), m)
        ...
        if before.channel is not None and after.channel is None:
            # left
            await send(
                f'{emoji}\U0001f4e4 {describe(member)} left {describe(before.channel)}'
            )
        elif before.channel is None and after.channel is not None:
            # joined
            await send(
                f'{emoji}\U0001f4e5 {describe(member)} joined {describe(after.channel)}'
            )
        elif before.channel != after.channel:
            # moved
            await send(
                f'{emoji}\U0001f504 {describe(member)} moved from {describe(before.channel)} to '
                f'{describe(after.channel)}')

    async def on_message_censor(self, filter: CensorshipFilter,
                                msg: discord.Message):
        # we don't want to log message deletes for this message
        self.censored_messages.add(message_id=msg.id)

        content = f': {msg.content}' if getattr(filter, 'show_content',
                                                True) else ''
        fmt = (
            f'\u002a\u20e3 Message by {describe(msg.author)} in {describe(msg.channel, mention=True)} censored: '
            f'{filter.mod_log_description}{content}')
        await self.log(msg.guild, fmt)

    async def on_member_autorole(self, member: discord.Member,
                                 roles_added: 'List[discord.Role]'):
        # make embed
        msg = (
            f'\N{BOOKMARK} Automatically assigned roles to {describe(member)}'
            if isinstance(roles_added, list) else
            f'\N{CLOSED BOOK} Failed to automatically assign roles for {describe(member)}'
        )

        if roles_added:
            # if roles were added, add them to the message
            msg += ', added roles: ' + ', '.join(
                describe(role) for role in roles_added)
    ...
    async def on_member_update(self, before: discord.Member,
        ...
        if before.nick != after.nick:
            ...
            if await self.autorole_debounces.check(
                    member_id=before.id,
                    role_ids=[role.id
                              for role in added_roles]) and not removed_roles:
                return

            fmt_before = f'\N{KEY} Roles for {describe(before)} were updated'
            fmt_diffs = describe_differences(self.bot, added_roles,
                                             removed_roles)
            ...
            await self.autoformat_responsible(msg, before,
                                              'member_role_update', formatter)
    ...
    async def on_raw_bulk_message_delete(self, message_ids, channel_id: int):
        ...
        await self.log(
            channel.guild,
            f'\U0001f6ae {len(message_ids)} message(s) deleted in {channel.mention}'
        )
    ...
    async def on_message_delete(self, msg: discord.Message):
        ...
        if await self.bulk_deletes.check_batch(
                msg.id
        ) or await self.censored_messages.check(message_id=msg.id):
            return
    ...
    async def on_guild_role_create(self, role: discord.Role):
        ...
        await self.autoformat_responsible_nontarget(
            msg, action='role_create', base=base)

    async def on_guild_role_delete(self, role: discord.Role):
        base = f'\N{KEY} {self.bot.tick("red", guild=role.guild)} Role deleted: {describe(role)}'
        msg = await self.log(role.guild, base)
        await self.autoformat_responsible_nontarget(
            msg, action='role_delete', base=base)

    async def on_member_join(self, member: discord.Member):
        new = '\N{SQUARED NEW} ' if (
            datetime.datetime.utcnow() -
            member.created_at).total_seconds() <= 604800 else ''
        await self.log(member.guild,
                       f'\N{INBOX TRAY} {new}{describe(member, created=True)}')

    def format_member_departure(self,
                                member: discord.Member,
                                *,
                                verb: str = 'left',
                                emoji: str = '\N{OUTBOX TRAY}') -> str:
        """
        ...
        bounce = '\U0001f3c0 ' if (
            datetime.datetime.utcnow() -
            member.joined_at).total_seconds() <= 1500 else ''
    ...
    async def get_responsible(
            ...
            action: str,
            *,
            target: discord.Member = None) -> discord.AuditLogEntry:
        """
        ...
        try:
            ...
            entries = await guild.audit_logs(
                limit=1, action=getattr(discord.AuditLogAction,
                                        action)).flatten()

            # only check for entries performed on target, and happened in the last 2 seconds
            def check(entry):
                created_ago = (datetime.datetime.utcnow() -
                               entry.created_at).total_seconds()
                return (entry.target == target
                        if target else True) and created_ago <= 2
    ...
    async def autoformat_responsible_nontarget(
            self, log_message: discord.Message, *, action: str, base: str):
        """
        ...
        if action:
            await log_message.edit(
                content=self.modlog_msg(f'{base} by {describe(action.user)}'))
    ...
    async def autoformat_responsible(self,
        ...
        audit_log_entry = await self.get_responsible(
            log_message.guild, action, target=targeted)
        ...
        if departure:
            ...
            fmt = self.format_member_departure(
                targeted, verb=verb, emoji=departure_emoji)
            await log_message.edit(content=self.modlog_msg(fmt))
        elif format_to:
            await log_message.edit(
                content=self.modlog_msg(format_to(audit_log_entry)))
    ...
    async def on_member_ban(self, guild: discord.Guild, user: discord.Guild):
        ...
        msg = await self.log(guild,
                             self.format_member_departure(
                                 user, verb=verb, emoji='\N{HAMMER}'))
        await self.autoformat_responsible(
            msg,
            user,
            'ban',
            departure=True,
            departure_extra=verb,
    ...
    async def on_member_unban(self, guild: discord.Guild, user: discord.User):
        ...
        await self.autoformat_responsible(
            msg, user, 'unban', format_to=formatter)
    ...
    async def on_member_remove(self, member: discord.Member):
        ...
        if await self.ban_debounces.check(
                user_id=member.id, guild_id=member.guild.id):
            return

        msg = await self.log(member.guild,
                             self.format_member_departure(member))
    ...
    async def is_public(self, ctx, channel: discord.TextChannel = None):
        ...
        await ctx.send(
            public.format('is' if await is_publicly_visible(self.bot, channel)
                          else '**is not**'))
