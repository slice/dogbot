...
class About(Cog):
    ...
    async def invite(self, ctx: DogbotContext):
        ...
        if ctx.bot.is_private:
            return await ctx.send("This bot is private and can't be invited. Sorry!")

        link = discord.utils.oauth_url(ctx.bot.user.id, permissions=discord.Permissions.none())
    ...
    async def wiki(self, ctx):
        ...
        await ctx.send(
            await ctx._('cmd.wiki', wiki=wiki, invite=invite)
        )
